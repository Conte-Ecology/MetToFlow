...
setwd("C:/ALR/Models/CTRflows3/get_flow_data/daily_seasonal")  #hard coded directory name...change me
...
for (i in 1:dim(dannual1)[[2]]) {
     j<-gages.met[gages.met$site_no== dimnames(dannual1)[[2]][i],"met_filename"]
     print(j)
     if (length(j)>0) {
...
save(dannual,file=file.path(model_data_dir,"dannual.Rdata"))
```

```{r seasonal}
dseasonal1<-array(dim=c(dim(qseasonal)[1],nrow(gages.met.spatial),6))
dimnames(dseasonal1)[[1]]<-dimnames(qseasonal)[[1]]
dimnames(dseasonal1)[[2]]<-gages.met.spatial$site_no
dimnames(dseasonal1)[[3]]<-c("rain","melt","precip.e.lag1","precip.e.lag2","precip.e.lag3","flow")
dseasonal1[,,"flow"]<-qseasonal[,gages.met.spatial$site_no,1]
for (i in 1:dim(dseasonal1)[[2]]) {
     j<-gages.met[gages.met$site_no== dimnames(dseasonal1)[[2]][i],"met_filename"]
     print(j)
     if (length(j)>0) {
          dseasonal1[,i,1:5]<-cseasonal[,j,2:6]
     }
}
